define dmlite::dpm::vo(
  ...
  exec { "ns_vo_${domain}_${name}":
    ...
    command => "dmlite-shell -e \"mkdir ${vopath} p\" && dmlite-shell -e \"groupadd ${name}\" && sleep 60 && dmlite-shell -e \"chgrp  ${vopath} ${name}\" && dmlite-shell -e \"chown ${vopath} root\" && dmlite-shell -e \"chmod ${vopath} 775\"",
